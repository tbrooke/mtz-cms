(ns mtz-cms.ui.layouts
  "Layout system for composing components into page structures

   Layouts define how components are arranged on a page.
   Each layout is a function that takes page-data and returns Hiccup.

   Architecture:
   - Layouts are pure presentation - they arrange components
   - Components come from components/ directory (hero.clj, feature.clj, etc.)
   - This file consolidates all layouts from layouts/templates.clj
   - HTMX-specific loading is separate in components/htmx.clj"
  (:require
...
(defn hero-features-layout
  "Layout: Hero section followed by features in a grid
   ...
   Used for: Home page, landing pages
   Components: Hero + Features grid + Call-to-action"
  [page-data]
...
(defn simple-content-layout
  "Layout: Simple content with sidebar
   ...
   Used for: About pages, general content pages
   Components: Main content + Sidebar with quick links"
  [page-data]
...
(defn cards-grid-layout
  "Layout: Grid of cards
   ...
   Used for: Events, ministries, news listings
   Components: Card grid with title and description"
  [page-data]
...
(defn render-layout
  "Render page with the specified layout
   ...
   Args:
     layout-type - Keyword identifying layout (:hero-features-layout, etc.)
     page-data - Map of page data to pass to layout

   Returns:
     Hiccup vector for the selected layout

   Example:
     (render-layout :hero-features-layout {:home/hero {...} :home/features [...]})"
...
(def available-layouts
  "Registry of available layouts for Alfresco selection
   ...
   Each layout entry describes:
   - :name - Human-readable layout name
   - :description - What this layout does
   - :components - What components it uses
   - :suitable-for - What page types it works well with

   Future: This registry can be exposed via API for
   Alfresco content editors to select layouts."
...
(comment
  ...
  (render-layout :hero-features-layout
                 {:home/hero {:hero/title "Welcome"
                             :hero/content "Test content"}
                  :home/features [{:feature/title "Feature 1"
                                  :feature/content "Content 1"}]})

  ;; Test simple content layout
  (render-layout :simple-content-layout
                 {:page/content "Test page content"})

  ;; List available layouts
  (keys available-layouts))
